import { wordObj } from "./utils/words";
...
let alpha = Alpha.rs('E', 18).rs('A', 13).rs('I', 12).rs('O', 11).rs('TR', 9).rs('N', 8).rs('DSU', 6).rs('L',5).rs('G', 4).rs('BCFHMPVWY', 3).rs('JKQXZ', 2).s()
...
class Game {
        ...
        Object.keys(look).forEach(lookKey => {
            ...
            Object.keys(orientation).forEach(orientationKey => {
                ...
                while (true) {
                    ...
                    // if out of bounds
                    if (this.tilemap[currentY] === undefined || this.tilemap[currentY][currentX] === undefined) break;
    ...
    getValidAdjecentTiles(x, y) {
        console.log('getting valid adject tiles');
        const startX = x;
        const startY = y;
        ...
        if (this.inBounds(...top) && this.getTile(...top)) {
            validTiles.push(top);
        }
        if (this.inBounds(...bottom) && this.getTile(...bottom)) {
            validTiles.push(bottom)
        }
        if (this.inBounds(...left) && this.getTile(...left)) {
            validTiles.push(left)
        }
        if (this.inBounds(...right) && this.getTile(...right)) {
